...
#define MAX_RIDES 10
/* Variables */
pthread_mutex_t check_in_lock; // mutex to control access of the
//'boarded' variable
pthread_mutex_t riding_lock; // mutex to control access of the
//'unboarded' variable
sem_t board_queue; // semaphore to ensure boarding of C
//passenger threads
sem_t all_boarded; // binary semaphore to tell passenger threads to
//wait for the next ride
sem_t unboard_queue; // semaphore to ensure unboarding of C
//passenger threads
sem_t all_unboarded; // binary semaphore to signal passenger
//threads for boarding
volatile int boarded = 0; // current number of passenger threads that
//have boarded
volatile int unboarded = 0; // current number of passenger threads
//that have unboarded
volatile int current_ride = 0; // current number of rides
volatile int total_rides; // total number of coaster rides for the
//instance
...
//Signal C passenger threads to board the car
sem_wait(&all_boarded); // Wait for all passengers to
//board
run();
unload();
for(i = 0; i < capacity; i++) sem_post(&unboard_queue); 
//Signal the passengers in the car to unboard
sem_wait(&all_unboarded); // Tell the queue to start
//boarding again
...
// Run indefinitely, threads will be destroyed when the main
//process exits
while(1){
sem_wait(&board_queue); // Wait for the car thread
//signal to board
pthread_mutex_lock(&check_in_lock); // Lock access to
//shared variable before incrementing
boarded++;
board();
if (boarded == capacity){
sem_post(&all_boarded); // If this is the last
//passenger to board, signal the car to run
boarded = 0;
}
pthread_mutex_unlock(&check_in_lock); // Unlock access
//to shared variable
sem_wait(&unboard_queue); // Wait for the ride to end
pthread_mutex_lock(&riding_lock); // Lock access to
//shared variable before incrementing
unboarded++;
unboard();
if (unboarded == capacity){
sem_post(&all_unboarded); // If this is the last
//passenger to unboard, signal the car to allow boarding
unboarded = 0;
}
pthread_mutex_unlock(&riding_lock); // Unlock access to
//shared variable
}
 return NULL;
}
/* Main program */
int main() {
// Set new instance of passenger threads, car capacity and total
//rides values
srand(time(NULL));
...
for(i = 0; i < passengers; i++) pthread_create(&passenger[i],
NULL, passengerThread, NULL);
